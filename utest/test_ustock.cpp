...
#define REQUEST_CMD "http://db2015.wstock.cn/wsDB_API/kline.php?symbol=%s&desc=1&q_type=0&r_type=2&etime=%s&stime=%s&u=test&p=8e6a"
...
public:
    uhttpclient(uschedule & schedule, uurl & url, 
        ...
        , schedule_(schedule)
        , url_(url)
        , decompress_buf_(new unsigned char[4096])
        , stock_ids_(ids)
        , stime_(stime)
        , etime_(etime)
    {
    ...
    uhttpresponse get(uhttp & http, const char * stock, 
        const char * sdate, const char *edate)
    {
        char buf[1024];
        snprintf(buf, 1024, REQUEST_CMD, stock, edate, sdate);
        ...
        //decomporess
        if (strcmp(response.get_header("Content-Encoding"), "gzip") == 0) {
        ...
        if (response.statuscode() == uhttp_status_ok) {
            ...
            printf("get stock(%s) succeed!(%s)\n", stock, content.c_str());
        } else {
            printf("get stock(%s) error!, info:%s\n", stock, content.c_str());
        }
        
#if 0
        std::fstream output("./sh600522", 
    ...
    void run()
    {
        ...
        int size = stock_ids_.size();
        for (int i = 0; i < 1; i++) {
            get(*http, stock_ids_[i].c_str(), stime_.c_str(), etime_.c_str());
            sleep(1);
        }
...
private:
    ...
    const std::vector<std::string> & stock_ids_;
...
int main(int argc, char * argv[])
{
    ...
    std::string stime, etime, filename, market;
    ...
    while((ch = getopt(argc, argv, "s:e:f:m:")) != -1) {
        switch(ch) {
            ...
            case 'e':
                ...
                break;
            case 'f':
                std::string(optarg).swap(filename);
                break;
            case 'm':
    ...
    printf("stime:%s, etime:%s, filename:%s\n", 
        stime.c_str(), etime.c_str(), filename.c_str());

    if (!read_stock_ids(filename, market, stock_ids)) {
        return -1;
    }

    printf("stock_ids:%zd, first id:%s\n", 
        stock_ids.size(), stock_ids.empty() ? "NONE" : stock_ids[0].c_str());
